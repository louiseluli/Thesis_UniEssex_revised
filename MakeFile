...
.PHONY: help env reqs fmt lint check all eda models fairness dissertation presentations \
        ...
        collect selfcheck install-schedules commit clean
...
# --- Automated data collection & pipeline scheduling
collect:  ## Incremental API collection: picks up new videos (newest-first, skips known IDs)
	$(PY) $(SRC)/data/collector.py --ordering newest --new-only

selfcheck:  ## Run key pipeline steps in selfcheck mode (safe, ~15 min, no DB overwrite)
	bash scripts/pipeline_selfcheck.sh

install-schedules:  ## Register daily collection + weekly selfcheck with macOS launchd
	@mkdir -p logs
	@chmod +x scripts/collect_daily.sh scripts/pipeline_selfcheck.sh
	cp scripts/com.louisesfer.thesis.collect.plist ~/Library/LaunchAgents/
	cp scripts/com.louisesfer.thesis.selfcheck.plist ~/Library/LaunchAgents/
	launchctl load ~/Library/LaunchAgents/com.louisesfer.thesis.collect.plist
	launchctl load ~/Library/LaunchAgents/com.louisesfer.thesis.selfcheck.plist
	@echo “Scheduled: collection daily at 03:00, selfcheck weekly Sunday at 04:00”
